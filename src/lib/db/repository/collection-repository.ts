 ...
 * 语言使用标准语言码（预设 EN/SC/TC/JP/KR + 自定义语言），状态按语言行记录
 ...
 * - 写入前校验变体确实存在于 card_prints，语言码必须是预设或已注册的自定义语言；
...
export async function upsertLangQty(
  ...
  if (!prints.some((p) => p.card_no_extend === cardNoExtend)) {
  ...
  if (!collectionId) {
    ...
    await db.execute(
      `INSERT INTO ${TABLES.COLLECTION}
       (id, card_id, card_no_extend, series_code, last_edited_at, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [collectionId, cardId, cardNoExtend, seriesCode, now(), now(), now()]
    )
  }
...
export async function setLangStatus(
  ...
  const langRows = await db.select<{ id: string; collection_id: string }[]>(
    `SELECT cl.id, cl.collection_id FROM ${TABLES.COLLECTION_LANGS} cl
     ...
     WHERE col.card_id = ? AND col.card_no_extend = ? AND cl.language_code = ?`,
    [cardId, cardNoExtend, code]
  )
  const existing = langRows[0]
  if (existing) {
    await db.execute(
  ...
  if (!collectionId) {
    ...
    await db.execute(
      `INSERT INTO ${TABLES.COLLECTION}
       (id, card_id, card_no_extend, series_code, last_edited_at, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [collectionId, cardId, cardNoExtend, deriveSeriesCode(cardNoExtend), now(), now(), now()]
    )
  }
  await db.execute(
    `INSERT INTO ${TABLES.COLLECTION_LANGS}
...
export async function createCustomPrint(input: CustomPrintInput): Promise<string> {
  ...
  if (input.normalQty > 0 || input.foilQty > 0) {
    await upsertLangQty(input.cardId, input.cardNoExtend, code, {
  ...
  if (cardIds.length > 0) {
    const ownedRows = await db.select<any[]>(
      `SELECT col.card_id, SUM(cl.normal_qty + cl.foil_qty) AS owned
       FROM ${TABLES.COLLECTION} col
       JOIN ${TABLES.COLLECTION_LANGS} cl ON cl.collection_id = col.id
       WHERE col.card_id IN (${cardIds.map(() => '?').join(',')}) AND cl.status = 'owned'
...
/** 清理孤儿收藏行（card_id 不在 cards_base，或 (card_id, card_no_extend) 组合不存在于 card_prints） */
...
/** 批量校验 items 对应的变体确实存在于 card_prints，过滤无效项 */
async function filterValidItems(items: CollectionItem[]): Promise<CollectionItem[]> {
  ...
  for (const item of items) {
    const key = `${item.cardId}|${item.cardNoExtend}`
    if (!unique.has(key)) unique.set(key, item)
  }
  const list = [...unique.values()]
  if (list.length === 0) return []
  const ids = [...new Set(list.map((i) => i.cardId))]
  const rows = await db.select<{ id: string; card_no_extend: string }[]>(
    `SELECT DISTINCT card_id AS id, card_no_extend FROM ${TABLES.CARD_PRINTS}
     WHERE card_id IN (${ids.map(() => '?').join(',')})`,
    ids
  )
  const exists = new Set(rows.map((r) => `${r.id}|${r.card_no_extend}`))
  for (const item of list) {
    if (exists.has(`${item.cardId}|${item.cardNoExtend}`)) valid.push(item)
  }
  return valid
}
...
async function loadExistingLangs(
  ...
  const conds = items.map(() => '(col.card_id = ? AND col.card_no_extend = ?)').join(' OR ')
  const params: any[] = []
  for (const i of items) params.push(i.cardId, i.cardNoExtend)
  const rows = await db.select<any[]>(
    `SELECT col.card_id, col.card_no_extend, cl.language_code, cl.status, cl.normal_qty, cl.foil_qty
     FROM ${TABLES.COLLECTION} col
     JOIN ${TABLES.COLLECTION_LANGS} cl ON cl.collection_id = col.id
     WHERE ${conds}`,
    params
  ...
  >()
  for (const r of rows) {
    const key = `${r.card_id}|${r.card_no_extend}`
...
export async function bulkIncrement(items: CollectionItem[]): Promise<number> {
  ...
  for (const item of valid) {
    ...
    if (ownedRows.length > 0) {
      ...
      await upsertLangQty(item.cardId, item.cardNoExtend, pick.language_code, {
...
export async function bulkDeleteCollection(items: CollectionItem[]): Promise<number> {
  ...
  const conds = valid.map(() => '(card_id = ? AND card_no_extend = ?)').join(' OR ')
  const params: any[] = []
  for (const i of valid) params.push(i.cardId, i.cardNoExtend)
...
export async function getRecentCollectionCards(limit = 6): Promise<RecentCollectionCard[]> {
  const db = await getDatabase()
  const rows = await db.select<any[]>(
    `SELECT col.card_id, col.card_no_extend, col.series_code, col.last_edited_at,
     ...
     FROM ${TABLES.COLLECTION} col
     JOIN ${TABLES.CARDS_BASE} cb ON cb.id = col.card_id
     LEFT JOIN ${TABLES.CARD_PRINTS} p ON p.card_id = col.card_id AND p.is_default = 1
  ...
  const sql = `SELECT cb.id AS card_id, cb.card_no, cb.card_name_cn,
     ...
     FROM (
       ...
       FROM (
         ...
         LEFT JOIN ${TABLES.COLLECTION} col ON col.card_id = p3.card_id AND col.card_no_extend = p3.card_no_extend
