...
import { buildOrderBy, mapRowToCard, mapRowToPrint, toSqliteModel } from './helper'
...
export async function saveVersion(version: AppVersion) {
  const db = await getLocalDb()
  await db.execute(`INSERT OR REPLACE INTO version (id, name, updated_at) VALUES ($1, $2, $3);`, [
    version.id,
    version.name,
    version.updated_at,
  ])
...
export async function getLocalCardBaseCount(): Promise<{
  ...
  const db = await getLocalDb()
  const cardsBaseCount = await db.select<number[]>('SELECT COUNT(*) FROM cards_base;')
  const cardsPrintCount = await db.select<number[]>('SELECT COUNT(*) FROM cards_print;')
  return {
...
export async function searchCardsLocal(params: CardSearchParams): Promise<CardSearchResult> {
  const db = await getLocalDb()
  ...
  for (const field of arrayFields) {
    ...
    if (filterParam.must && filterParam.must.length > 0) {
      for (const val of filterParam.must) {
        whereClauses.push(`EXISTS (SELECT 1 FROM json_each(cards_base.${field}) WHERE value = ?)`)
  ...
  const textFields: (keyof CardSearchParams)[] = ['card_category', 'series_name', 'rarity_name']
...
export async function getFilterOptions(): Promise<FilterOptions | null> {
  const db = await getLocalDb()
  const rows = await db.select<any[]>('SELECT * FROM filter_options WHERE id = 1;')
