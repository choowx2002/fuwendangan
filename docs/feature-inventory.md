...
| F02 | 账号体系（登录/多用户/云端身份） | 跨设备身份与数据归属 | 未实现 | 无 auth 相关表/代码；`player-data-sync.md` 明确「无账号体系，anon key 直连」 | 否 | 否（Beta 边界） | Beta 期间按单机单用户定位 |
| F03 | 多设备数据同步 | 换机/多端数据一致 | 未实现 | `docs/player-data-sync.md`（设计定稿、待实施）；无 user_sync_* 表 | 否 | 否（用备份/导出替代） | 见 missing-features M01 |
...
| F55 | 到期系统通知（推送提醒） | 主动提醒 | 未实现 | 无 reminder/notification 定时逻辑 | 否 | 否 | 目前仅应用内角标/汇总 |
...
| F98 | 清单 CSV 回导 | 迁移 | 未实现 | 仅有导出无导入解析 | 否 | 否 | |
...
| F115 | 借还/联系人 CSV 导出 | 备份 | 未实现 | 无 loans/contacts 导出 | 否 | 否 | 整库备份已覆盖 |

## M. 备份 / 恢复

| ID | 功能名称 | 用户价值 | 状态 | 证据来源 | 核心 | 影响Beta | 备注 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| F120 | 整库备份（.db 另存） | 防丢数据 | 已实现 | `settings/+page.svelte` `backupDatabase`（关库复制） | 是 | 是 | 手动；需用户主动 |
| F121 | 整库恢复（.db 导入） | 换机/回滚 | 已实现 | `restoreDatabase`（有确认） | 是 | 是 | 无 schema 版本校验，旧备份恢复有风险 |
| F122 | 自动备份/定期提醒 | 防丢数据 | 未实现 | 无定时/启动备份逻辑 | 否 | 是（Beta 需告知手动备份） | |
| F123 | 设置项（settings.json）纳入备份 | 还原偏好 | 未实现 | 备份仅含 SQLite；store 文件不在备份范围 | 否 | 否 | 换机后需重设偏好 |
...
| F151 | 规则列表空状态 | 无数据提示 | 未实现 | `/rules` 无空态（空则白页） | 否 | 否 | |
| F152 | 系列收藏页空状态 | 无数据提示 | 未实现 | `/collection/[seriesCode]` 网格无空态 | 否 | 否 | |
...
| F161 | 应用内反馈表单/崩溃上报 | 结构化收集问题 | 未实现 | 无 | 否 | 否 | 可用日志 + 问卷代替 |
...
| F173 | 联系人私密信息本机保存 | 隐私 | 已实现 | `contacts` 表本地；无云端 | 是 | 是 | 备份文件含此信息，需提醒勿外传 |
| F174 | 应用自动更新 | 安全修复分发 | 未实现 | 无 updater 插件 | 否 | 是（Beta 需手动分发新包） | |
...
| F190 | 收藏卡片历史撤销/重做 | 误操作保护 | 部分实现 | 历史页有撤销；无重做 | 否 | 否 | |
